...
私人任务规则写在配置目录旁的 `TASK_RULES.md`，内容不会进仓库或状态库正文。推荐把
本机安装实例里不含秘密的配置备份到仓库 `/.local/owner-config/`；该目录已被
gitignore，换上下文时优先在这里找本地实例材料。运行中的配置仍以
`~/.config/lark-agent/` 为准，token 和密钥不要放进这个备份。群里
...
显式恢复会开启新的通信代际：旧回复、Owner 通知和调查进度仍可审计，但不会替新一代
完成处理；资源写入等有副作用的动作继续沿用各自的幂等保护。
