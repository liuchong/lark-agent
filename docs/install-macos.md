...
安装后，Owner 可在智能助手私聊中发送 `/help`、`/status`、`/tasks`、`/memory`
和 `/rules` 查询运行状态、待处理任务、持久记忆和私人任务规则状态。任务详情会给出精确的恢复、取消、确认、核对
...
同一条消息不混用两种说明语言。
新安装会在配置目录写入通用 `TASK_RULES.md` 并启用；已有安装保持关闭，直到本机执行
`lark-agent rules init`。规则正文是私人配置，不会写入公开仓库或 SQLite 正文。
