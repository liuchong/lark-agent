...
问题时，剩余的有界代码工具仍可继续使用；全部字段已有证据时应立即提交结论。有可引用
证据后，最后两个模型回合只允许提交结论和进行一次有界修正，不能继续消耗调查轮次。
...
保持原始大小写并限制在该子树；无法携带路径范围的全局索引或调用链工具会被拒绝。
模型传入配置工作区目录名前缀时会规范化为工作区相对路径；`search_workspace` 未显式
传入 `path` 时由运行时注入该精确子树。搜索先匹配完整短语，未命中时要求空格分隔的
每个查询词都出现在同一文件中，仍不会扫描或返回精确子树以外的兄弟项目。
