...
使用不同的幂等动作；重启后不会重复已完成的进度消息。最终必须变成有证据的结果、
Owner 已处理或明确阻塞，不能只留下“正在调查”。
私聊方向判断优先区分“对方提出新请求”和“对方回答 Owner 的问题”。只有目标消息
本身包含新的问题、请求、邀请或协调义务时，普通真人私聊才会进入调查或代回复；
回答、确认、继续 Owner 发起的话题且没有新增义务时会记录为无需回复。群聊条件不变：
只有原生 `@Owner` 才能进入代回复。
...
- 主模型已形成且通过发送前校验的回复，会在最后一次“Owner 是否已处理”检查前保存为
  回复候选。上下文仍含糊时只复查会话，不重跑模型。`/task 工作号` 会明确显示
  “尚未回复原提问者”、未发送草稿、已核对、未知和下一步；随后确认仍未回复时才按
  原有幂等链路发送一次，发现 Owner 已处理或原消息撤回则取消候选。重新领取后会先
  重跑当前暂停、黑名单和允许范围规则；规则已收紧时直接取消旧候选。候选的写入、持
  有、消费和取消都受当前工作租约约束，过期进程不能改写新一轮调查结果。
...
显式恢复表示重新调查：它会取消尚未发送的旧回复候选，不会把旧草稿直接发给原
提问者。普通 daemon 重启不会取消候选；恢复后仍只复查当前会话上下文。
