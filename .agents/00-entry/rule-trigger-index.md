# Rule trigger index

Always read `AGENTS.md` first.

- plan, BDD, TDD, implementation, regression:
  `/workspace/rules/bdd-tdd-development-flow.md`
- Lark, Feishu, IM, event, identity, credential:
  `.agents/knowledge/standalone-boundary.md`
- shell, Workspace, permissions, security:
  `AGENTS.md` hard boundaries and `spec/behavior.md`
- persistence, session, recovery, resume, schema migration:
  `spec/behavior.md` and `changes/standalone-repository-rewrite.md`
- install, deploy, restart, launchd:
  `docs/install-macos.md` and `docs/operations.md`
- context resume or handoff:
  current plan, `git status`, validation evidence, and active `.agents` records
